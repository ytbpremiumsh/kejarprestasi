  ...
  return <div className="space-y-5">
    ...
    <section className={cn(adminPanelClass,"overflow-hidden")}>
      ...
      {loading ? <div className="grid min-h-48 place-items-center"><Loader2 className="h-6 w-6 animate-spin text-violet-600"/></div> : filtered.length===0 ? <div className="py-16 text-center text-sm text-slate-500">Belum ada pengiriman esai yang sesuai.</div> : <>
        ...
        <div className="space-y-3 p-3 md:hidden">{filtered.map(row=><button key={row.key} onClick={()=>setSelected(row)} className="w-full rounded-2xl border border-violet-100 p-4 text-left"><div className="flex items-start justify-between gap-2"><div><p className="font-bold text-indigo-950">{row.registration?.full_name??"Peserta tidak ditemukan"}</p><p className="mt-1 font-mono text-xs text-slate-500">{row.token}</p></div><CategoryPill kind={row.kind??row.registration?.kind??"umum"}/></div><div className="mt-4 flex items-center justify-between border-t border-violet-100 pt-3 text-xs text-slate-500"><span>{row.answers?.filter(Boolean).length??0}/5 jawaban</span><span className="font-bold text-violet-700">Lihat detail</span></div></button>)}</div>
      </>}
    </section>
